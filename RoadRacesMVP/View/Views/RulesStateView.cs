...
namespace RoadRacesMVP
{
    public class RulesStateView : StateView
    {
        ...
        {
            spriteBatch.Draw(backgrounds[(byte)ViewType.RulesStateView], new Vector2(0, 0), Color.White);
            foreach (var c in StateComponents)
            {
                ...
                {
                    var x = (c.Rectangle.X + (c.Rectangle.Width / 2)) - (font45.MeasureString(c.Text).X / 2);
                    var y = (c.Rectangle.Y + (c.Rectangle.Height / 2)) - (font45.MeasureString(c.Text).Y / 2);

                    spriteBatch.DrawString(font45, c.Text, new Vector2(x, y), Color.Black);
                }
            }
        }
